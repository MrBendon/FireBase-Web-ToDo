<template>
  <!-- <base-card v-if="getisInProgress" class="loginPage"> -->
  <base-card class="loginPage">
        ...
        <div class="aboutBox">
          <div class="about__text">
            ...
            <ul class="about__list">
              ...
              <li>Vuex</li>
              <li>Vue Router</li>
              <li>Firebase Authentication、Firestore and Hosting</li>
            </ul>
          </div>
    ...
    <div class="Login__title">
      <div class="infoImg">
        <img @click="toggleshowAbout" src="../../../assets/information.png" alt="" />
      </div>
      <div>Just do it</div>
      <div>And</div>
    ...
    <form @submit.prevent="login" class="Form-Control">
      <div>
        ...
        <transition name="alert" mode="out-in">
          ...
          <div v-else-if="LoginErrorMessage && isAccountError && EmailInput" class="alert">
            {{ LoginErrorMessage }}
          </div>
      ...
      <div class="PWinputBox">
        ...
        <transition name="alert" mode="out-in">
          ...
          <div v-else-if="LoginErrorMessage && !isAccountError" class="alert">{{ LoginErrorMessage }}</div>
          <div v-else>&nbsp;</div>
        </transition>
      </div>
      <div class="midBox">
        <router-link class="SignUp" :isLink="true" to="/signup">前往註冊新帳號</router-link>
        <p @click="ForgetPage" class="SignUp" :isLink="false">忘記密碼</p>
      </div>
      <div class="buttonBox">
        <base-button @click.prevent="login">登入</base-button>
        <!-- <base-button @click="logout">登出測試</base-button> -->
      </div>
    </form>
    <br />
  </base-card>
  <forget-pw v-if="goForgetPage" @cancel="cancel"></forget-pw>
</template>
...
export default {
  ...
  computed: {
    ...
    LoginErrorMessage() {
      // console.log(this.$store.getters.getLoginErrorMessage);
      return this.$store.getters["AuthModule/getLoginErrorMessage"];
    },
    isAccountError() {
      if (this.LoginErrorMessage.includes("帳號")) {
        return true;
      } else {
        return false;
      }
  ...
  methods: {
    ...
    login() {
      ...
      this.$store.dispatch("AuthModule/Login", { email, password });
      this.$store.dispatch("AuthModule/changeIsLoading", true);
    },
    cancel() {
      this.goForgetPage = false;
      this.sentCheck = false;
      this.showAbout = false;
    },
    ForgetPage() {
      this.goForgetPage = true;
    },
  },
